...
{
    private IBlockState blockStateWood = ModBlocks.log_banana.getDefaultState();
    private IBlockState blockStateLeaves = ModBlocks.leaves_banana.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
    private IBlockState blockStateFruit = ModBlocks.banana_fruit.getDefaultState();
     ...
     */
    public WorldGenTreesBanana(boolean parShouldNotify)
    {
    ...
    private void generateLeaves(World parWorld, BlockPos parBlockPos, int height, Random parRandom)
    {
        for (int foliageY = parBlockPos.getY() - 3 + height; foliageY <= parBlockPos.getY() + height; ++foliageY)
        {
            ...
            int foliageLayerRadius = 1 - foliageLayer / 4;
            ...
            for (int foliageX = parBlockPos.getX() - foliageLayerRadius; foliageX <= parBlockPos.getX() + foliageLayerRadius; ++foliageX)
            {
                ...
                for (int foliageZ = parBlockPos.getZ() - foliageLayerRadius; foliageZ <= parBlockPos.getZ() + foliageLayerRadius; ++foliageZ)
                {
                    ...
                    if (Math.abs(foliageRelativeX) != foliageLayerRadius || Math.abs(foliageRelativeZ) != foliageLayerRadius || parRandom.nextInt(4) != 0 && foliageLayer != 0)
                    {
                        ...
                        int iPosY = parBlockPos.getY() + height - 3;
                        BlockPos fruitBlockPos = new BlockPos(foliageX, iPosY - 1, foliageZ);
                        BlockPos blockBelowFruitPos = new BlockPos(foliageX, iPosY - 2, foliageZ);
                        if (parWorld.isAirBlock(fruitBlockPos) && fruitBlockPos.getX() != parBlockPos.getX() && fruitBlockPos.getZ() != parBlockPos.getZ()) {
                            if (parWorld.isAirBlock(blockBelowFruitPos) && iPosY > 2) {

                                if (parRandom.nextInt(3) == 0) {
                                    this.setBlockAndNotifyAdequately(parWorld, fruitBlockPos, blockStateFruit);
                                }
